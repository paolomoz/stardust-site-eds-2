...
- `scripts/cube-ui.js` + `styles/cube-ui.css`: triggers and discoverability. Press and hold on the page, or start moving horizontally, and the cube is in your hand on both axes. On release the dominant axis commits past the threshold or on a flick, the other springs back. A plain vertical drag stays a scroll. Drag an edge handle inward on any side. The widget is a slim tag on the right edge at mid-height, with a strip of the five other faces' seed colours and a chevron; hovering it (tapping, on touch) slides it open into a panel with a small live cube whose faces are miniatures of the pages in their palettes. It plays one slow demo turn on arrival and is dragged to turn the page; focus it and use the arrow keys. On mouse out the panel slides back into the edge while the cube keeps moving. Nothing interrupts the first screen. Anything the UI adds is marked `data-cube-exclude` and never ends up inside a face.
...
- Tuning: `--cube-duration`, `--cube-easing`, `--cube-pullback`, `--cube-perspective`, `--cube-void` in `styles/cube.css`; `--cube-edge-size`, `--cube-edge-glow`, `--cube-widget-size`, `--cube-tag-width/height/open` in `styles/cube-ui.css`; `--rubik-seam` in `styles/cube-rubik.css`; gesture thresholds at the top of `scripts/cube-ui.js` and `scripts/cube-rubik.js`.
